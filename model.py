...
boxnum = tf.maximum(tf.reduce_sum(obj_t),1.)

detector_out = tf.layers.conv2d(tmp,25,(1,1))#[None,7,7,25]
xy = tf.nn.sigmoid(detector_out[...,0:2])
wh = tf.sqrt(tf.clip_by_value(tf.nn.sigmoid(detector_out[...,2:4]),1e-2,1.))
...
objerr = tf.reduce_mean((obj-obj_t)**2 * tf.where(tf.equal(obj_t,1.),tf.ones_like(obj_t)*1,tf.ones_like(obj_t)*1))
