...
def constructlabel(im, lbmap, dim):
    ...
    fms = dim//32
    xy = np.ones(shape=[fms,fms,2])*0.5
    wh = np.zeros(shape=[fms,fms,2])
    obj = np.zeros(shape=[fms,fms,1])
    ...
    for ob in objs:
        ...
        xmax = ob[3]
        ymax = ob[4]
        w = np.sqrt((xmax-xmin)*ratio/dim)
        h = np.sqrt((ymax - ymin) * ratio / dim)
        ...
        #print(1)
    #newim.show()
    return (np.array(newim),xy,wh,obj,cls)
